...
struct RowMultiply<L1,List<T,TT...>> {
	typedef typename PrependList < Int<SumList< typename ListMultiply<L1,T>::result>::value>,
		 typename RowMultiply<L1,List<TT...>>::result>::list result;
};

template <typename L1,typename L2>
struct RowMultiply<L1,List<List<L2>>>{
	typedef  List<Int<SumList< typename ListMultiply<L1,List<L2>>::result>::value>> result;
};
